...
public final class NetworkImage extends LeafComponent<Image, NetworkImage> {
	...
	private static ImageLoader loader = new DefaultImageLoader();

	/** GL texture upload. Must be called on the main thread. */
	static TextureRegion toTexture(Pixmap pixmap) {
		try {
	...
	public static void setImageLoader(ImageLoader customLoader) {
		loader = customLoader != null ? customLoader : new DefaultImageLoader();
	}
	...
	private void loadUrl(@Nullable String url) {
		...
		TextureRegionDrawable cached = cache.get(cacheKey(url, radius, targetW));
		if (cached != null) {
		...
		loader.load(url, radius, targetW, targetH,
				region -> applyResult(url, radius, targetW, gen, region),
				error -> applyFailure(gen, url));
	}

	/**
	 * Single success path for every load: caches the result regardless of
	 * currency, then applies it only if this load is still the latest one.
	 */
	private void applyResult(String url, int radius, float targetW, long gen, TextureRegion region) {
	...
	/**
	 * Single failure path for every load: marks the component failed and shows
	 * the fallback (or placeholder) only if this load is still the latest one.
	 */
	private void applyFailure(long gen, String url) {
		if (isCurrent(gen, url)) {
			this.failed = true;
			applyDrawable(fallback != null ? fallback : placeholder);
		}
	...
	/**
	 * Attempts to serve the load from the disk cache (component-level so it
	 * applies to every loader). Corrupt entries are deleted and the load falls
	 * back to the network within the same load call. Returns true when the
	 * load has been handed off to the disk path.
	 */
		...
		try {
			DECODE_WORKER.execute(() -> {
				...
				try {
					...
					retryNetwork(url, radius, targetW, targetH, gen);
					return;
				}
	...
	// --- Disk cache (shared with DefaultImageLoader) ---
