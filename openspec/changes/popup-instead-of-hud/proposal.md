...
- In popup mode with QuickAccess on: tapping the QuickAccess button toggles the popup (opens if closed, hides if already showing) instead of toggling feature enablement; the standalone HUD is fully suppressed; long-press still opens settings; enable/disable lives only in the settings dialog and FeatureCard.
...
- The popup has no title header; instead, it renders the shared control row inside a container styled with a black background, `rounded(unit(2))`, `padding(unit(1))`, and `gap(unit(1))`.
- When the popup is showing, clicking the QuickAccess button again hides the popup.
