...
- In popup mode with QuickAccess on: tapping the QuickAccess button opens a stacked mini-panel popup instead of toggling; the standalone HUD is fully suppressed; long-press still opens settings; enable/disable lives only in the settings dialog and FeatureCard.
...
- Net clients opening the TimeControl popup see the controls visibly disabled.
