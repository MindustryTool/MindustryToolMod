...
### Decision 3: Signal lives inside, `show(data, x, y)` is atomic
Alternatives were a caller-owned signal (`shared.set(req); menu.showAt(x, y)`) or a Solim-internal singleton. Caller-owned splits one user gesture across two calls; a Solim singleton bakes global state into shared infra (lifecycle + test-reset burden forever). `Popup<T>` owns a private signal; `show(data, x, y)` sets it, positions, and shows in one call; an in-`build()` effect renders content via the provider, positions, attaches listeners. Re-showing while open hot-swaps content and re-anchors for free. Public visibility is strictly explicit — the signal is never an auto-show trigger.
...
Existing `Popup` style already chains (`rounded()`/`border()` return `this`); all new methods (`children(provider)`, `show()`, `hide()`) return `Popup<T>`. One known implementation-time check: generic inference through `popup().children(req -> ...)` against an assignment target — javac supports target-type propagation through chains, with an explicit type witness as fallback.
