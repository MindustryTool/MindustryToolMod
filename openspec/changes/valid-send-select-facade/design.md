...
- **Compute `canSend` directly from `messageText` plus `isValidInput`.** `canSend` stays where it was at the top of `build()` as a `Computed` over `!isSending && isValidInput(messageText)` — no field capture, no dependency on the field's `valid()` signal. The supplier MUST use tracked `messageText.get()` (not `peek()`), otherwise the `Computed` never re-evaluates on keystrokes. Rejected: capturing the `SolimTextField` to read `input.valid()` (extra coupling for identical outcomes — both signals update on every change).
