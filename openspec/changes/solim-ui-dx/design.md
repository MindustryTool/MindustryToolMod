...
- **Lifecycle preserved.** `BaseComponent` constructor registers the pending child at factory time. `build()` tolerates a not-yet-supplied factory and renders empty; when `.children(...)` arrives after the element was built (e.g. via eager `CellConfig` calls such as the constructor's own `growX()`, or user chaining `.grow()` before `.children(...)`), the component refreshes its already-built content directly. A minimal `BaseComponent.isBuilt()` accessor supports this; without it the desired API is impossible because `CellConfig` modifiers force `element()` during configuration. No new subscriptions, effects, allocations, or reconciliation passes; the fluent form is field assignment plus at most one content refresh.
