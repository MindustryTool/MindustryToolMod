...
public class PlayerConnectUI {
    public static class Filter { public volatile String text = ""; public volatile boolean showHidden = false; }
    ...
    public static void buildHeader(Table hosts, Collapser coll, Runnable onShowHidden) {
        hosts.table(n -> {
            ...
            n.button(Icon.eyeSmall, Styles.emptyi, onShowHidden).update(i -> i.getStyle().imageUp = Icon.eyeSmall).size(40f).right().padRight(3).tooltip("@servers.showhidden");
            n.button(Icon.add, Styles.emptyi, () -> new CreateRoomDialog().show()).size(40f).right().padRight(3).tooltip("Create Room");
            n.button(Icon.downOpen, Styles.emptyi, () -> { coll.toggle(false); Core.settings.put("collapsed-playerconnect", coll.isCollapsed()); })
    ...
    public static void buildSearch(Table hosts, Filter filter, Runnable refresh) {
        hosts.table(f -> {
            ...
            f.check("@servers.remote.hidden", filter.showHidden, b -> { filter.showHidden = b; refresh.run(); }).update(c -> c.setChecked(filter.showHidden)).padLeft(10);
