...
public class LoginDialog extends BaseDialog {
    ...
    private void rebuild() {
        ...
        if (isLoggingIn) { cont.add("@login.waiting").pad(20f).row(); cont.add("@login.browser-hint").color(Color.gray).pad(10f).row(); cont.button("@cancel", Icon.cancel, this::cancelLogin).size(200f, 60f).pad(10f); }
        else if (AuthService.isLoggedIn()) {
            cont.add("@login.logged-in").color(Color.green).pad(20f).row();
            String token = AuthService.getAccessToken();
            if (token != null && !token.isEmpty()) { cont.add("Token:").pad(5f).row(); cont.table(t -> { t.field(token, s -> {}).disabled(true).width(380f).get(); t.button(Icon.copy, () -> { arc.Core.app.setClipboardText(token); Vars.ui.showInfoFade("Token copied"); }).size(40f); }).pad(5f).row(); }
            cont.button("@login.logout", Icon.cancel, this::doLogout).size(200f, 60f).pad(10f);
        }
        else { cont.add("@login.not-logged-in").pad(20f).row(); cont.button("@login.button", Icon.play, this::startLogin).size(200f, 60f).pad(10f); }
        cont.row();
        cont.button("Clear Cache", Icon.trash, this::clearAllCache).size(200f, 50f).pad(10f).padTop(30f);
    }
    
    private void clearAllCache() {
        ImageCache.clear(); NetworkImage.clearCache();
        Main.imageDir.deleteDirectory(); Main.mapsDir.deleteDirectory(); Main.schematicDir.deleteDirectory();
        Main.imageDir.mkdirs(); Main.mapsDir.mkdirs(); Main.schematicDir.mkdirs();
