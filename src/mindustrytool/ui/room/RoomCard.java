...
public class RoomCard {
    ...
    public static void renderList(Table container, Seq<PlayerConnectRoom> roomList, boolean loading, String search) {
        ...
        if (loading) { container.table(Tex.button, t -> t.add("[accent]" + Core.bundle.get("message.loading")).pad(10f)).growX().row(); return; }
        if (roomList.isEmpty()) { container.table(Tex.button, t -> t.add("@hosts.none").pad(10f)).growX().row(); return; }

        Seq<PlayerConnectRoom> filtered = filterRooms(roomList, search);
        if (filtered.isEmpty()) { container.table(Tex.button, t -> t.add("@hosts.none").pad(10f)).growX().row(); return; }
    ...
    private static Button buildButton(PlayerConnectRoom room, float width) {
        ...
        float tw = width - 40f;
        String ver = "[accent]v" + room.data().version(), lock = room.data().isSecured() ? " [scarlet]" + Iconc.lock : "";
        Table hdr = new Table(Tex.whiteui); hdr.setColor(Pal.gray);
        btn[0].add(hdr).height(45f).growX().row();
        hdr.add(room.data().name() + "   " + ver + lock).left().padLeft(10f).wrap().style(Styles.outlineLabel).growX();
        hdr.button(Icon.copy, Styles.emptyi, () -> { Core.app.setClipboardText(room.data().name()); Vars.ui.showInfoFade("@copied"); }).margin(3f).pad(8f).padRight(4f).top().right().tooltip("Copy");
    ...
    private static Seq<PlayerConnectRoom> filterRooms(Seq<PlayerConnectRoom> rooms, String search) {
        ...
        return rooms.select(r -> {
            String name = r.data().name().toLowerCase(), map = r.data().mapName().toLowerCase(), mode = r.data().gamemode().toLowerCase();
            return name.contains(s) || map.contains(s) || mode.contains(s);
        });
    }

    private static String getVersionString(String s) {
        int build = -1; String type = "custom";
        if (!"custom build".equals(s)) { Matcher m = VERSION_PATTERN.matcher(s); if (m.matches()) { type = m.group(1); build = Integer.parseInt(m.group(2)); } }
        if (build == -1) return Core.bundle.format("server.custombuild");
        if (build == 0) return Core.bundle.get("server.outdated");
        if (Version.build != -1) {
            if (build < Version.build) return Core.bundle.get("server.outdated") + "\n" + Core.bundle.format("server.version", build, "");
            if (build > Version.build) return Core.bundle.get("server.outdated.client") + "\n" + Core.bundle.format("server.version", build, "");
        }
        return (build == Version.build && Version.type.equals(type)) ? "" : Core.bundle.format("server.version", build, type);
    }
}
