...
public class TagService {
    public enum TagCategoryEnum { schematics, maps }
    ...
    public void getTag(TagCategoryEnum cat, Cons<Seq<TagCategory>> l) {
        var item = categories.get(cat.name());
        if (item != null) { Core.app.post(() -> l.get(item)); return; }
        fetch(cat, (tags) -> { categories.put(cat.name(), tags); Core.app.post(() -> l.get(tags)); });
    }

    private void fetch(TagCategoryEnum cat, Cons<Seq<TagCategory>> l) {
        Http.get(Config.API_URL + "tags?group=" + cat)
            .error(e -> { Log.err(Config.API_URL + "tags", e); Core.app.post(() -> l.get(new Seq<>())); })
            .submit(r -> result(r, l));
    }
    ...
    private void result(Http.HttpResponse r, Cons<Seq<TagCategory>> l) {
        ...
        Core.app.post(() -> { l.get(tags); onUpdate.run(); });
    }
