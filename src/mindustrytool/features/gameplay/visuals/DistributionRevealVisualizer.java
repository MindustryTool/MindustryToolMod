package mindustrytool.features.gameplay.visuals;
...
public class DistributionRevealVisualizer {
    ...
    public void showSettings() {
        if (dialog == null) {
            dialog = new BaseDialog(Core.bundle.get("mdt.distribution.settings.title", "Distribution Reveal Settings"));
            dialog.addCloseButton();

            dialog.buttons.button("@mdt.settings.reset", mindustry.gen.Icon.refresh, () -> {
                resetToDefaults();
                rebuildDialog();
            }).size(210, 64);
    ...
    private void rebuildDialog() {
        ...
        // Title
        cont.add(Core.bundle.get("mdt.distribution.settings.title", "Distribution Reveal Settings"))
                .color(Color.white).padBottom(10).row();

        // === Toggles ===
        cont.check(Core.bundle.get("mdt.distribution.showItemBridges", "Show Item Bridges"), showItemBridges, val -> {
            showItemBridges = val;
            saveSettings();
        }).left().padTop(4).row();

        cont.check(Core.bundle.get("mdt.distribution.showLiquidBridges", "Show Liquid Bridges"), showLiquidBridges,
                val -> {
                    showLiquidBridges = val;
                    saveSettings();
                }).left().padTop(4).row();

        cont.check(Core.bundle.get("mdt.distribution.showRouters", "Show Routers & Distributors"), showRouters, val -> {
            showRouters = val;
            saveSettings();
        }).left().padTop(4).row();

        cont.check(Core.bundle.get("mdt.distribution.showUnloaders", "Show Unloaders"), showUnloaders, val -> {
            showUnloaders = val;
            saveSettings();
        }).left().padTop(4).row();

        cont.check(Core.bundle.get("mdt.distribution.showArrows", "Show Direction Arrows"), showArrows, val -> {
            showArrows = val;
            saveSettings();
        }).left().padTop(4).row();

        cont.check(Core.bundle.get("mdt.distribution.showBottleneck", "Highlight Bottlenecks (Red)"), showBottleneck,
                val -> {
                    showBottleneck = val;
                    saveSettings();
                }).left().padTop(4).row();
        ...
        thicknessContent.add(Core.bundle.get("mdt.distribution.lineThickness", "Line Thickness"), Styles.outlineLabel)
                .left().growX();
        ...
        arrowContent.add(Core.bundle.get("mdt.distribution.arrowSize", "Arrow Size"), Styles.outlineLabel).left()
                .growX();
        ...
        zoomContent.add(Core.bundle.get("mdt.distribution.zoomThreshold", "Min Zoom"), Styles.outlineLabel).left()
                .growX();
