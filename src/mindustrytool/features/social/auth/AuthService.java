...
public class AuthService {
    ...
    private static final long REFRESH_MARGIN_MS = 60_000, LOGIN_TIMEOUT_MS = 30_000;
    private static final String ACCESS_KEY = "mindustrytool.auth.accessToken",
            REFRESH_KEY = "mindustrytool.auth.refreshToken";
    ...
    public static String getAccessToken() {
        if (accessToken != null && isExpired(accessTokenExpiry)) {
            if (refreshToken != null)
                refreshTokenAsync(null, null);
            return null;
        }
        if (accessToken != null && shouldRefresh(accessTokenExpiry, REFRESH_MARGIN_MS))
            refreshTokenAsync(null, null);
        return accessToken;
    }
    ...
    public static void login(Cons<String> ok, Cons<String> err) {
        ...
        Http.get(API_URL + "auth/app/login-uri")
                ...
                .submit(r -> handleLoginUri(r, t -> {
                    setTokens(t);
                    ok.get("Login successful!");
                }, err));
    }
    ...
    private static void handleLoginUri(Http.HttpResponse r, Cons<TokenResponse> ok, Cons<String> err) {
        try {
            ...
            if (u.loginUrl == null || u.loginId == null) {
                post(err, "Invalid response");
                return;
            }
            Core.app.post(() -> Core.app.openURI(u.loginUrl));
            pollToken(u.loginId, ok, err, System.currentTimeMillis());
        } catch (Exception e) {
            Log.err("[Auth] URI error", e);
            post(err, "Error: " + e.getMessage());
        }
    }

    private static void pollToken(String id, Cons<TokenResponse> ok, Cons<String> err, long start) {
        if (System.currentTimeMillis() - start > LOGIN_TIMEOUT_MS) {
            post(err, "Login timeout");
            return;
        }
    ...
    private static void retryPoll(String id, Cons<TokenResponse> ok, Cons<String> err, long start, Throwable e) {
        if (System.currentTimeMillis() - start < LOGIN_TIMEOUT_MS)
            Time.runTask(60f, () -> pollToken(id, ok, err, start));
        else
            post(err, "Failed: " + e.getMessage());
    }

    private static void handleTokenResponse(Http.HttpResponse r, String id, Cons<TokenResponse> ok, Cons<String> err,
            long start) {
        try {
            ...
            Core.app.post(() -> ok.get(t));
        } catch (Exception e) {
            Log.err("[Auth] Token error", e);
            post(err, "Error: " + e.getMessage());
        }
    }
    ...
    public static void refreshTokenAsync(Cons<Boolean> ok, Cons<String> err) {
        if (refreshToken == null || refreshToken.isEmpty()) {
            if (err != null)
                Core.app.post(() -> err.get("No refresh token available"));
            return;
        }
        if (isRefreshing) {
            if (ok != null)
        ...
        Http.post(API_URL + "auth/app/refresh", "{\"refreshToken\":\"" + refreshToken + "\"}")
                ...
                .error(e -> {
                    isRefreshing = false;
                    Log.err("[Auth] Refresh failed", e);
                    if (err != null)
                        Core.app.post(() -> err.get("Session expired"));
                })
                .submit(res -> {
                    isRefreshing = false;
                    try {
                        ...
                        if (tokens.accessToken != null) {
                            ...
                            setTokens(tokens);
                            if (ok != null)
                                Core.app.post(() -> ok.get(true));
                        } else {
                            logout();
                            if (err != null)
                                Core.app.post(() -> err.get("Failed to refresh"));
                        }
                    } catch (Exception e) {
                        Log.err("[Auth] Error parsing refresh", e);
                        logout();
                        if (err != null)
                            Core.app.post(() -> err.get("Error: " + e.getMessage()));
                    }
                });
    }
    ...
    public static void logout() {
        ...
        arc.Events.fire(new mindustrytool.events.LoginStateChangeEvent(false));
    }
    ...
    private static void setTokens(TokenResponse t) {
        ...
        if (accessToken != null)
            Core.settings.put(ACCESS_KEY, accessToken);
        else
            Core.settings.remove(ACCESS_KEY);
        if (refreshToken != null)
            Core.settings.put(REFRESH_KEY, refreshToken);
        else
            Core.settings.remove(REFRESH_KEY);

        arc.Events.fire(new mindustrytool.events.LoginStateChangeEvent(isLoggedIn()));
    }

    private static void loadTokens() {
        accessToken = Core.settings.getString(ACCESS_KEY, null);
        refreshToken = Core.settings.getString(REFRESH_KEY, null);
        if (accessToken != null) {
            accessTokenExpiry = parseExpiry(accessToken);
    ...
    private static long parseExpiry(String token) {
        ...
        try {
            ...
            // Use rigorous JSON parsing instead of fragile string manipulation
            arc.util.serialization.JsonValue json = new arc.util.serialization.JsonReader().parse(payload);
            if (json.has("exp")) {
    ...
    private static boolean isExpired(long expiryMs) {
        return expiryMs > 0 && System.currentTimeMillis() > expiryMs;
    }

    private static boolean shouldRefresh(long expiryMs, long marginMs) {
        return expiryMs > 0 && System.currentTimeMillis() > expiryMs - marginMs;
    }

    private static void post(Cons<String> c, String m) {
        Core.app.post(() -> c.get(m));
    }
}
