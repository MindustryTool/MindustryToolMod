...
/** Dialog for browsing and joining Player Connect rooms. */
public class PlayerConnectRoomsDialog extends BaseDialog {
    ...
    public PlayerConnectRoomsDialog() {
        super("@mdt.message.room-list.title");
        addCloseButton();
        cont.table(root -> {
    ...
    private void renderUI() {
        roomList.clear();
        if (isLoading.get()) { roomList.add(Core.bundle.get("mdt.message.loading")).center().pad(20f); roomList.invalidateHierarchy(); return; }
        Seq<PlayerConnectRoom> rs = rooms.get();
        if (rs.isEmpty()) { roomList.add(Core.bundle.get("mdt.message.no-rooms-found")).center().pad(20f); roomList.invalidateHierarchy(); return; }
