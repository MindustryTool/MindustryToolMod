...
public class JoinDialogInjector {
    ...
    public static void inject() {
        Runnable hook = () -> {
            needsRefresh = true;
            Core.app.post(JoinDialogInjector::injectSection);
        };
        Vars.ui.join.shown(hook);
        Vars.ui.join.resized(hook);
        Runnable render = () -> RoomCard.renderList(roomsContainer, rooms.get(), isLoading.get(), filter.text);
        rooms.bind(render);
        isLoading.bind(render);
    }

    private static void injectSection() {
        // Don't inject if PlayerConnect is disabled
        if (!PlayerConnectFeature.getRoomsDialog().isEnabled()) {
            // Remove existing section if disabled
    ...
    private static void build(Table parent) {
        ...
        PlayerConnectUI.buildHeader(parent, coll, () -> {
            filter.showHidden = !filter.showHidden;
            refreshRooms();
        });
        Table section = new Table();
        PlayerConnectUI.buildSearch(section, filter, JoinDialogInjector::refreshRooms);
    ...
    private static void refreshRooms() {
        isLoading.set(true);
        Api.findPlayerConnectRooms(filter.text, f -> {
            rooms.set(f == null ? new Seq<>() : f);
            isLoading.set(false);
        });
    }
