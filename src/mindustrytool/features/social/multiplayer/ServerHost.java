...
/** Represents a server host with IP and port parsing. */
public class ServerHost {
    ...
    public boolean set(String ip) {
        ...
        last = ip;
        if (ip.isEmpty()) {
            error = "@mdt.message.room.missing-host";
        ...
        try {
            ...
            if (isIpv6 && (idx = ip.lastIndexOf("]:")) != -1 && idx != ip.length() - 1) {
                ...
                error = "@mdt.message.room.missing-port";
                return wasValid = false;
            }
            if (port < 0 || port > 0xFFFF) throw new Exception();
            return wasValid = true;
        } catch (Exception e) {
            error = "@mdt.message.room.invalid-port";
            return wasValid = false;
        }
    }
