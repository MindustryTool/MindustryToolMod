...
public class BrowserSettingsDialog extends BaseDialog {
    ...
    public BrowserSettingsDialog(ContentType type, Runnable onSettingsChanged) {
        super(Core.bundle.get("mdt.message.lazy-components.settings", "Settings") + " - " + type.name());
        this.settings = new FeatureSettings("browser." + type.name().toLowerCase());
        this.onSettingsChanged = onSettingsChanged;

        addCloseButton();
        buttons.button("@mdt.settings.reset", Icon.refresh, this::resetToDefaults).size(250, 64);
    ...
    private void rebuild() {
        ...
        cont.pane(t -> {
            ...
            // === Display Section ===
            addSectionHeader(t, Core.bundle.get("mdt.settings.section.display", "Display"));

            // Items per page - slider
            addSliderRow(t,
                    Core.bundle.get("mdt.settings.items-per-page", "Items per page"),
            ...
            addSliderRow(t,
                    Core.bundle.get("mdt.settings.card-scale", "Card Size"),
    ...
    private void addSliderRow(Table table, String label, int current, int min, int max, int step,
        ...
        // Calculate width like Mindustry (passed from parent or recalculated?
        // Recalculated is fine for stack)
    ...
    public static int getItemsPerPage(ContentType type) {
        return new FeatureSettings("browser." + type.name().toLowerCase()).getInt(ITEMS_PER_PAGE, 20);
    }

    public static int getCardWidth(ContentType type) {
        return new FeatureSettings("browser." + type.name().toLowerCase()).getInt(CARD_WIDTH, 300);
    }

    public static int getCardHeight(ContentType type) {
        return new FeatureSettings("browser." + type.name().toLowerCase()).getInt(CARD_HEIGHT, 200);
    }

    public static boolean getShowPreview(ContentType type) {
        return new FeatureSettings("browser." + type.name().toLowerCase()).getBool(SHOW_PREVIEW, true);
    }
}
