...
public final class NetworkImageLoader {
    ...
    public static void load(String url, ThreadSafeCache<String, TextureRegion> cache, Runnable onError) {
        ...
        if (url.startsWith("https://api.mindustry-tool.com"))
            return; // Don't cache/load API results as images

        Fi file = BrowserDirInit.imageDir
                .child(url.replace(":", "-").replace("/", "-").replace("?", "-").replace("&", "-"));
        if (file.exists())
            loadFile(url, file, cache, onError);
        else
            loadHttp(url, file, cache, onError);
    }

    private static void loadFile(String url, Fi file, ThreadSafeCache<String, TextureRegion> cache, Runnable onError) {
        try {
            createTexture(file.readBytes(), url, cache, () -> {
                file.delete();
                onError.run();
            });
        } catch (Exception e) {
    ...
    private static void createTexture(byte[] data, String url, ThreadSafeCache<String, TextureRegion> cache,
            Runnable onError) {
        try {
            ...
            Core.app.post(() -> {
                try {
                    ...
                    cache.put(url, new TextureRegion(t));
                    pix.dispose();
                } catch (Exception e) {
                    pix.dispose();
                    onError.run();
