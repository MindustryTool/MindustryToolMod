...
import mindustrytool.features.content.browser.*;
...
public class DetailDialog extends BaseBrowserDialog {
    ...
    private void addComment(Table table, mindustrytool.features.content.browser.CommentData c) {
        Table row = new Table();
    ...
    private void buildHero(Object data) {
        ...
        mindustrytool.features.content.browser.ImageHandler.ImageType imgType = (data instanceof MapDetailData)
                ? mindustrytool.features.content.browser.ImageHandler.ImageType.MAP
                : mindustrytool.features.content.browser.ImageHandler.ImageType.SCHEMATIC;

        previewTable.add(new mindustrytool.features.content.browser.ImageHandler(id, imgType))
        ...
        headerTable
                .button(mindustry.gen.Icon.link, mindustry.ui.Styles.clearNonei,
                        () -> Core.app.openURI(mindustrytool.features.content.browser.Config.WEB_URL +
        ...
        if (data instanceof SchematicDetailData) {
            ...
            mindustrytool.features.content.browser.Api.downloadSchematic(id, bytes -> {
                try {
                    if (bytes == null || bytes.length == 0)
                        return;
                    String schemString = new String(arc.util.serialization.Base64Coder.encode(bytes));
                    mindustry.game.Schematic sc = mindustrytool.features.content.browser.SchematicUtils
        ...
        if (mobile) {
            ...
            // Date
            String timeStr = mindustrytool.features.content.browser.SchematicUtils.parseRelativeTime(createdAt);
        ...
        if (data instanceof SchematicDetailData) {
            mindustrytool.features.content.browser.Api.findSchematicById(id, fullData -> {
                if (fullData != null) {
                    Core.app.post(() -> {
    ...
    private void loadUser(String userId, Table avatarContainer, arc.scene.ui.Label nameLabel) {
        if (userId != null) {
            ...
            mindustrytool.features.content.browser.Api.findUserById(userId, user -> {
                if (user != null) {
                    Core.app.post(() -> {
            ...
            mindustrytool.features.content.browser.ApiRequest.getJval(
                    mindustrytool.features.content.browser.Config.API_URL + "users/" + userId,
                    json -> {
                        Core.app.post(() -> {
    ...
    private void populateRequirements(Table container, Iterable<?> reqs) {
        ...
        for (Object o : reqs) {
            ...
            if (o instanceof mindustry.type.ItemStack) {
                ...
                amount = stack.amount;
                item = stack.item;
            } else if (o instanceof mindustrytool.features.content.browser.SchematicDetailData.SchematicRequirement) {
                mindustrytool.features.content.browser.SchematicDetailData.SchematicRequirement r = (mindustrytool.features.content.browser.SchematicDetailData.SchematicRequirement) o;
                amount = r.amount();
                name = r.name();
    ...
    private void buildActionBar(Object data, String id) {
        ...
        actionTable.button(mindustry.gen.Icon.upOpen, mindustry.ui.Styles.defaulti,
                () -> {
                    if (mindustrytool.features.content.browser.BrowserAuthService.getAccessToken() == null) {
                        mindustry.Vars.ui.showInfo("Please login first.");
                        return;
                    }
                    mindustrytool.features.content.browser.Api.vote(
        ...
        if (data instanceof MapDetailData) {
            actionTable.button(mindustry.gen.Icon.download, mindustry.ui.Styles.defaulti,
                    () -> {
                        mindustrytool.features.content.browser.ContentHandler.downloadMap(
                                new mindustrytool.features.content.browser.ContentData().id(((MapDetailData) data).id()));
                    })
                    .tooltip("@download");
        } else {
            // Copy Base64
            actionTable.button(mindustry.gen.Icon.paste, mindustry.ui.Styles.defaulti,
                    () -> {
                        mindustrytool.features.content.browser.ContentHandler.copySchematic(
                                new mindustrytool.features.content.browser.ContentData().id(((SchematicDetailData) data).id()));
                    })
            ...
            actionTable.button(mindustry.gen.Icon.download, mindustry.ui.Styles.defaulti,
                    () -> {
                        mindustrytool.features.content.browser.ContentHandler.downloadSchematic(
                                new mindustrytool.features.content.browser.ContentData().id(((SchematicDetailData) data).id()));
                    })
    ...
    private void showComments(Object data, String id) {
        ...
        mindustrytool.features.content.browser.PagingRequest<mindustrytool.features.content.browser.CommentData> request = mindustrytool.features.content.browser.Api
                .getCommentsRequest(
        ...
        inputTable.button(mindustry.gen.Icon.pencil, mindustry.ui.Styles.defaulti, () -> {
            ...
            mindustrytool.features.content.browser.Api.postComment(
        ...
        request.getPage(items -> {
            Core.app.post(() -> {
                ...
                if (items == null || items.isEmpty()) {
                    ...
                    for (mindustrytool.features.content.browser.CommentData c : items) {
    ...
    private void showDetailsPopup(Object data) {
        ...
        mindustrytool.features.content.browser.Api.downloadMap(id, bytes -> {
            if (bytes == null || bytes.length == 0) {
                Core.app.post(() -> {
                    rulesTable.clear();
            ...
            try {
                mindustry.game.Rules rules = mindustrytool.features.content.browser.MapUtils.readRules(bytes);
                Core.app.post(() -> {
                    rulesTable.clear();
                    if (rules == null) {
    ...
    private void ensureTagsLoaded(Object data, Runnable onDone) {
        mindustrytool.features.content.browser.TagService service = new mindustrytool.features.content.browser.TagService();
        mindustrytool.features.content.browser.TagService.TagCategoryEnum type = (data instanceof MapDetailData)
                ? mindustrytool.features.content.browser.TagService.TagCategoryEnum.maps
                : mindustrytool.features.content.browser.TagService.TagCategoryEnum.schematics;
