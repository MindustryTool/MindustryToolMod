package mindustrytool.plugins.controls;
...
public class TouchSettingsDialog extends BaseDialog {
    ...
    private void setup() {
        ...
        // === Visuals Section ===
        table.add("@settings.touch.visuals").color(Pal.accent).left().padBottom(6f).row();
        ...
        // === Features Section ===
        table.add("@settings.touch.features").color(Pal.accent).left().padBottom(6f).row();
        table.image().color(Pal.accent).fillX().height(3f).padBottom(10f).row();
    ...
    private void refreshHandler() {
        if (TouchPlugin.touchComponent.isLoaded()) {
            TouchPlugin.touchComponent.get().rebuild();
        }
    }
