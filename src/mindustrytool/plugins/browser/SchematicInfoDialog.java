...
public class SchematicInfoDialog extends BaseDialog {
    ...
    private static ItemSeq toItemSeq(Seq<SchematicDetailData.SchematicRequirement> requirements) {
        ItemSeq result = new ItemSeq(); 
        if (requirements == null) return result;
        for (SchematicDetailData.SchematicRequirement req : requirements) { 
            if (req == null || req.name() == null || req.amount() == null) continue;
            Item item = (Item) content.getByName(ContentType.item, req.name()); 
