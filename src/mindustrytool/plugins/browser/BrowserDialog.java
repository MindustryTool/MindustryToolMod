...
public class BrowserDialog<T extends ContentData> extends BaseDialog {
    ...
    private static int calculateItemsPerPage(ContentType type) { 
        ...
        return Math.max((int)(Core.graphics.getWidth() / Scl.scl(s)) * (int)(Core.graphics.getHeight() / Scl.scl(IMG_SIZE + INFO_H * 2)), 20); 
    }
    ...
    private void initListeners() {
        ...
        onResize(() -> { 
            request.setItemPerPage(calculateItemsPerPage(type)); 
            rebuild(); 
            if (filterDialog.isShown()) filterDialog.show(searchConfig); 
        });
    }
    ...
    private void rebuild() {
        ...
        addCloseButton(); 
        row(); 
        BrowserSearchBarBuilder.build(this, search, options, request, searchConfig, debouncer, filterDialog, this::handleResult, this::hide); 
        row(); 
