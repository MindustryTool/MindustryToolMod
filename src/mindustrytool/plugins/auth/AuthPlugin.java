...
public class AuthPlugin implements Plugin {
    ...
    private static void createAccountButton() {
        accountTable = new Table(Styles.black5) {
            @Override public void act(float delta) { super.act(delta); updateButtonState(); updateSize(); }
        };
        ...
        if (AuthService.isLoggedIn()) fetchSession();
    }
    ...
    private static class DragHandler extends InputListener {
        ...
        DragHandler(Table target) { this.target = target; }
        ...
        @Override public void touchUp(InputEvent event, float x, float y, int pointer, KeyCode button) { isDragging = false; }
    }
}
