...
public class BackgroundSettingsDialog extends BaseDialog {
    ...
    public BackgroundSettingsDialog() {
        ...
        buttons.button("Reset to defaults", Icon.refresh, () -> {
            BackgroundPlugin.getInstance().clearBackground();
            setup();
        }).size(250f, 64f);
    ...
    private void setup() {
        ...
        BackgroundPlugin plugin = BackgroundPlugin.getInstance();
        if (plugin == null) {
            cont.add("Plugin not loaded!").color(mindustry.graphics.Pal.remove);
            return;
        }
        ...
        if (plugin.hasBackground()) {
            try {
                // Load texture for preview
                Fi file = plugin.getBackgroundFile();
                if (file.exists()) {
