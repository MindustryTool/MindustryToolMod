package mindustrytool.plugins.background;
...
public class BackgroundPlugin implements Plugin {

    private static BackgroundPlugin instance;
    ...
    public static BackgroundPlugin getInstance() {
        return instance;
    }
    ...
    public void init() {
        ...
        Log.info("[BackgroundPlugin] Initialized. Background file: @, exists: @", backgroundFile.path(),
    ...
    private void replaceRenderer() {
        ...
        try {
            ...
            if (rendererField != null) {
                ...
                Log.info("[BackgroundPlugin] Successfully replaced MenuRenderer.");
            } else {
                Log.err("[BackgroundPlugin] Could not find 'renderer' field in MenuFragment.");
            }

        } catch (Exception e) {
            Log.err("[BackgroundPlugin] Failed to replace MenuRenderer", e);
        }
    }
    ...
    public void setBackgroundFile(Fi file) {
        ...
        loadTexture();
        Log.info("[BackgroundPlugin] Set background: @ -> @", file.name(), backgroundFile.path());
    }
    ...
    private void loadTexture() {
        ...
        try {
            ...
            Log.info("[BackgroundPlugin] Loaded custom background: @ (@x@)", backgroundFile.name(),
                    backgroundTexture.width, backgroundTexture.height);
        } catch (Exception e) {
            Log.err("[BackgroundPlugin] Failed to load background", e);
        }
    }
