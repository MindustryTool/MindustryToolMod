...
public class SmartDrillManager {
    ...
    private void initSettings() {
        // Defaults
        if (!Core.settings.has(SETTING_MECH_MAX_TILES)) Core.settings.put(SETTING_MECH_MAX_TILES, 200);
        if (!Core.settings.has(SETTING_MECH_MIN_ORES)) Core.settings.put(SETTING_MECH_MIN_ORES, 1);
        
        if (!Core.settings.has(SETTING_PNEU_MAX_TILES)) Core.settings.put(SETTING_PNEU_MAX_TILES, 150);
        if (!Core.settings.has(SETTING_PNEU_MIN_ORES)) Core.settings.put(SETTING_PNEU_MIN_ORES, 2);
    ...
    private void registerEvents() {
        // Use TapEvent like the original mod - this is the key!
    ...
    private void registerDraw() {
        Events.run(EventType.Trigger.draw, () -> {
            if (!enabled || !showPreview || previewPlans.isEmpty()) return;
    ...
    private void updateSelectTable() {
        ...
        if (Blocks.mechanicalDrill.environmentBuildable() && ((Drill) Blocks.mechanicalDrill).canMine(selectedTile)) {
            addDrillButton(Blocks.mechanicalDrill, () -> {
                ...
                directionAction = dir -> executeAndQueue(DrillFiller.fillBridgeDrill(selectedTile, (Drill) Blocks.mechanicalDrill, dir));
            });
        }
        ...
        if (Blocks.pneumaticDrill.environmentBuildable() && ((Drill) Blocks.pneumaticDrill).canMine(selectedTile)) {
            addDrillButton(Blocks.pneumaticDrill, () -> {
                selectTable.visible = false;
                directionTable.visible = true;
                directionAction = dir -> executeAndQueue(DrillFiller.fillBridgeDrill(selectedTile, (Drill) Blocks.pneumaticDrill, dir));
            });
        }
        ...
        if (wallDrop != null || hasSandAttribute) {
            ...
            if (Blocks.cliffCrusher.environmentBuildable()) {
                boolean valid = false;
                // Cliff Crusher is WallCrafter, check attribute or wallDrop
                if (wallDrop != null) valid = true; // Simple check, assume tier 1 can mine anything or check hardness if needed
                if (hasSandAttribute) valid = true;

                if (valid) {
                    addDrillButton(Blocks.cliffCrusher, () -> {
                        selectTable.visible = false;
                        directionTable.visible = true;
                        directionAction = dir -> executeAndQueue(DrillFiller.fillWallDrill(selectedTile, Blocks.cliffCrusher, dir));
                    });
                }
            }
            
            // Plasma Bore - for other wall resources
            if (Blocks.plasmaBore.environmentBuildable() && wallDrop != null && wallDrop.hardness <= ((BeamDrill) Blocks.plasmaBore).tier) {
                addDrillButton(Blocks.plasmaBore, () -> {
                    selectTable.visible = false;
                    directionTable.visible = true;
                    directionAction = dir -> executeAndQueue(DrillFiller.fillWallDrill(selectedTile, (BeamDrill) Blocks.plasmaBore, dir));
                });
            }
        }
    ...
    private void executeAndQueue(Seq<BuildPlan> plans) {
        ...
        // Keep enabled = true so user can continue tapping without pressing keybind again
    }
    ...
    public void showSettings() {
        ...
        cont.pane(t -> {
            ...
            // Mechanical Drill
            addDrillSetting(t, "Mechanical Drill", Blocks.mechanicalDrill, SETTING_MECH_MAX_TILES, SETTING_MECH_MIN_ORES);
    ...
    public void draw() {
        // Drawing handled in registerDraw
    }
