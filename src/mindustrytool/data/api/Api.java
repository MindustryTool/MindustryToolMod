...
public class Api {
    public static void downloadSchematic(String id, ConsT<byte[], Exception> c) { Http.get(Config.API_URL + "schematics/" + id + "/data").submit(r -> c.get(r.getResult())); }
    public static void downloadMap(String id, ConsT<byte[], Exception> c) { Http.get(Config.API_URL + "maps/" + id + "/data").submit(r -> c.get(r.getResult())); }
    ...
    public static void findPlayerConnectRooms(String q, Cons<Seq<PlayerConnectRoom>> c) {
        Http.get(Config.API_v4_URL + "player-connect/rooms?q=" + q).submit(r -> {
    ...
    public static void findPlayerConnectProvider(Cons<Seq<PlayerConnectProvider>> p, Cons<Throwable> f) {
        Http.get(Config.API_v4_URL + "player-connect/providers").error(f).submit(r -> {
            String d = r.getResultAsString();
