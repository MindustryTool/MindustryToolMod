...
public class ProxyProvider implements NetProvider {
    ...
    public NetProvider getDelegate() {
        return delegate;
    }
    ...
    public void sendAllServer(Object object, Iterable<NetConnection> connections, boolean reliable) {
        ...
        for (NetConnection nc : connections) {
            if (nc instanceof ArcConnection && ((ArcConnection) nc).connection instanceof NetworkProxy.VirtualConnection) {
                NetworkProxy.VirtualConnection vc = (NetworkProxy.VirtualConnection) ((ArcConnection) nc).connection;
                if (vc.isConnected()) {
    ...
    public void sendExceptServer(NetConnection except, Object object, boolean reliable) {
        ...
        int excludeId = -1;
        if (except instanceof ArcConnection && ((ArcConnection) nc(except)).connection instanceof NetworkProxy.VirtualConnection) {
            excludeId = ((NetworkProxy.VirtualConnection) ((ArcConnection) nc(except)).connection).getID();
        }
        sendToAllVirtual(object, reliable, excludeId);
    }

    private NetConnection nc(NetConnection conn) {
        return conn;
    }
